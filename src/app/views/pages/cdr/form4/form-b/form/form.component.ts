...
import { postNeonatalDeathdReviewForm } from "./specs/form";
...
import { CdrForm4BService } from "../../../../../../services/cdr/form4b.service";
...
export class FormComponent implements OnInit, AfterViewInit {
	...
	onSubmit() {
		...
		console.log("[form 4B]", JSON.stringify(data));
		let isEdit = false;
		this.activatedRoute.params
			.pipe(
				...
				switchMap(({ id }) => {
					if (id) {
						isEdit = true;
						return this.cdrForm4BService.update(data, id);
					}
					return this.cdrForm4BService.add(data);
				})
			)
			.subscribe((response) => {
				...
				this.router.navigate(["/cdr/form4/b"]);
			});
	}
	...
	private initForm() {
		this.form = postNeonatalDeathdReviewForm;
	...
	ngOnInit() {
		...
		this.activatedRoute.params.pipe(take(1)).subscribe(({ id, cdrForm1Id }) => {
			if (id) {
				...
				this.cdrForm4BService.getOne(id).subscribe((response) => {
					...
					this.form.patchValue({ basic, sectionA: { address } }, { emitEvent: false });
					setTimeout(() => {
						this.changeDetectorRef.detectChanges();
					}, 500);
				});
	...
	backClicked() {
		this.router.navigateByUrl("/cdr/form4/b");
	}
}
