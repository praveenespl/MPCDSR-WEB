...
import { CdrForm3Service } from "../../../../../../services/cdr/form3.service";
...
export class FormComponent implements OnInit, AfterViewInit {
		...
		this.activatedRoute.params
			.pipe(
				...
				switchMap(({ id }) => {
					if (id) {
						isEdit = true;
						return this.cdrForm3Service.update(data, id);
					}
					return this.cdrForm3Service.add(data);
				})
			)
			.subscribe((response) => {
				...
				this.router.navigate(["/cdr/form3/a"]);
			});
	}
		...
		this.form.valueChanges
			...
			.subscribe(([prevValue, nextValue]) => {
				...
				if (changes.hasOwnProperty("sectionA")) {
					...
					// 	if (sectionA["place_of_death"] == "On way to health facility/in transit") {} else {
	...
	icd10CdrCode:any;
	ngOnInit() {
		this.today = moment().set({hours: 0, minutes: 0, seconds: 0, milliseconds: 0});
		this.initForm();
		...
		this.activatedRoute.params.pipe(take(1)).subscribe(({ id, cdrForm1Id }) => {
			...
			if (id) {
				this.cdrForm3Service.getOne(id).subscribe((response) => {
	...
	backClicked() {
		this.router.navigateByUrl("/cdr/form3/a");
	}
}
