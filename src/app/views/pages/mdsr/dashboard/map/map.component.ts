...
export class MapComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges {
	...
	ngOnInit() {
		...
		if (this.user.accessupto == "Block" && this.callingFrom == undefined) {
			...
			if (month > 3) {
				this.obj["fromDate"] = `${year}-03-31`;
				this.obj["toDate"] = `${year + 1}-04-01`;
				this.obj["year"] = `${year}-${year + 1}`;
			} else {
				this.obj["fromDate"] = `${year - 1}-03-31`;
				this.obj["toDate"] = `${year}-04-01`;
				this.obj["year"] = `${year - 1}-${year}`;
			}
	...
	getDeathsWhereCbmdsrAndFbmdsrConducted(passingArg) {
		...
		if (this.user.accessupto == "National" && this.callingFrom == undefined) {
			this.obj['type'] = "getStates";
			forkJoin(
				this.form1Service.getExpectedVsActual(passingArg),
				this.obj["type"] === "getStates" ? this.gisService.getStates({ type: this.obj["type"] }) : this.gisService.getDistricts({ type: this.obj["type"] })
			).subscribe(res => {
				this.stateData = [];
				this.stateData = res[0];
				if (this.obj["type"] === "getStates") {
					this.myOutput.emit(res[0]);
				}
			...
			this.obj['type'] = "getSubDistricts";
			forkJoin(this.form1Service.getDeathsWhereCbmdsrAndFbmdsrConducted(param),
				this.gisService.getSubDistricts({ type: this.obj['type'], states: [this.user.user_state_id['statename']], districts: this.callingFrom == true ? [this.passingArgOnDistrictClick['category']] : [this.passingArgOnDistrictClick['districtname']] })).subscribe(res => {
					this.cbmdsrAndFbmdsrDataForBlock = [];
					this.cbmdsrAndFbmdsrDataForBlock = res[0];
					this.generateMap(res[1], this.mapChartDivVar);
				})
	...
	generateMap(res, param) {
		// const colors = [
		//   am4core.color('#4A148C')
		// ];
		// if (this.chartMap) {
		//   this.chartMap.dispose();
		//   this.chartMap = undefined;
		// }
		...
		for (let i = 0; i < res[0].features.length; i++) {
			...
			if (this.obj["type"] === "getStates") {
				const foundState = this.stateData.find(s => {
					return s.state.toLowerCase() == itemMap.properties.name.toLowerCase();
				});
				data.push({
					id: itemMap.id,
					stateName: foundState ? foundState.state : '',
					...
					value: foundState ? foundState.actual : 0
				})
		...
		if (this.obj["type"] === "getStates") {
			statePolygon.tooltipText = "State Name: {stateName} \n Maternal Death Reported: {value}";
		} else if (this.obj["type"] === "getDistricts") {
			statePolygon.tooltipText = "District Name : {districtName} \n # Maternal Deaths : {value}\n # Deaths Reported in CBMDSR : {whereCBMDSRConducted} \n # Deaths Reported in FBMDSR : {whereFBMDSRConducted}";
		} else if (this.obj["type"] === "getSubDistricts") {
			...
			statePolygon.events.on("hit", (ev) => {
				...
				this.gisService.getDistricts({ stateName: data["name"] }).subscribe(res => {
					setTimeout(() => {
						...
						this.form1Service.getExpectedVsActual(this.obj).subscribe(districtsDeathReported => {
		...
		stateSeries.events.on("inited", function () {
			stateSeries.mapPolygons.each(function (polygon) {
				let label = labelSeries.mapImages.create();
				let state = polygon.dataItem.dataContext['blockName'];
		...
		stateSeries.events.on("inited", function () {
			for (var i = 0; i < data.length; i++) {
				var polygon = stateSeries.getPolygonById(data[i].id);
				if (polygon) {
					const label = labelSeries.mapImages.create();
					const blockName = polygon.dataItem.dataContext['blockName'];
					label.latitude = polygon.visualLatitude;
					label.longitude = polygon.visualLongitude;
					label.children.getIndex(0)['text'] = blockName
				}
			}
		});
