...
export class LayoutConfigService {
	...
	getSumanLogo(): string {
		const menuAsideLeftSkin = objectPath.get(this.layoutConfig, 'brand.self.skin');
		// set brand logo
		const logoObject = objectPath.get(this.layoutConfig, 'self.suman');
		...
		if (typeof logo === 'undefined') {
			try {
				const logos = objectPath.get(this.layoutConfig, 'self.suman');
				logo = logos[Object.keys(logos)[0]];
			} catch (e) {
			}
		}
	 ...
	 * Returns sticky logo
	 */
	getStickySumanLogo(): string {
		let logo = objectPath.get(this.layoutConfig, 'self.suman.sticky');
		if (typeof logo === 'undefined') {
			logo = this.getSumanLogo();
		}
		return logo + '';
	}
